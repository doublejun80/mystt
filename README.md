...
> 다음 컨텍스트에서 이어받을 때는 먼저 [NEXT_CONTEXT.md](NEXT_CONTEXT.md)를 읽으세요.
> 현재 실행 상태, 공개 QA 사용법, 최근 수정/검증, 디버그 명령을 한곳에 정리했습니다.
...
- 로컬 개발 기본 포트는 충돌을 피하려고 `web=3203`, `api=4100`, `postgres=55432`, `redis=56379`, `minio=19000/19001`, `mailpit=11025/18025`를 사용합니다.
...
- 웹 포털은 `/health`, `/ready`, `/v1/*`를 same-origin으로 API에 프록시하므로, 핸드폰 브라우저는 `3203` 한 포트만 보면 됩니다.
...
- `apps/desktop`은 Tauri 셸로 로컬 `http://127.0.0.1:3203` 또는 운영 `https://mystt.doublejun.digital` 포털을 앱 창에 띄우고, 설정 모달에서는 recorder 생존 상태와 persistence 진단을 다루는 노트북 레인입니다.
- desktop 실기동 전에는 `pnpm desktop:preflight`로 `4100` API, `3203` 로컬 포털, 선택적 `1420` dev shell 상태를 먼저 확인할 수 있습니다.
- 데스크톱 셸 설정에는 Postgres/MinIO 상태, 로컬 `.data` fallback 경로, recorder 루트, runtime-state 경로, sessions 인덱스, keep-awake(`caffeinate`) 상태가 표시됩니다.
...
- `/health`는 공개 liveness만 반환하고, `/ready`는 persistence 상태와 queue mode/depth를 포함해 worker lane을 점검합니다.
...
운영 메모, 검증 기준, rollback plan은 [docs/graphify.md](docs/graphify.md)에서 관리합니다.
...
- The local `.data` tree remains the development and outage fallback for state, artifacts, audio copies, and audit logs.
...
- 실기기 검증 없이 `화면 꺼짐 성공`으로 간주하지 않습니다. 검증 절차는 [infra/runbooks/mobile-native-recorder.md](infra/runbooks/mobile-native-recorder.md)에 정리합니다.
