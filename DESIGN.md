...
This file started from `npx getdesign@latest add linear.app --out DESIGN.md` and is intentionally adapted for `mystt`, a Korean meeting recorder with live captions, transcript correction, and instant meeting summaries.

## mystt Product Adaptation

- Keep the overall **Linear density and precision**, but tune it for a recording workflow rather than a generic SaaS dashboard.
- The main page is a **two-column product workspace**:
  - Left: the primary canvas. This is where users read live captions, inspect meeting summaries, and edit the full transcript.
  - Right: the compact control rail. This contains timer, recording controls, audio preview, and optional diagnostics.
- Never place long-form meeting summaries or full transcript editing in a narrow right sidebar. Those views must render in the large left workspace.
- The live caption panel must have a **fixed visual height** and internal scrolling. As more lines arrive, the panel scrolls inside itself; the page layout must not stretch downward.
- Favor **dense, compact controls** over oversized hero marketing sections. The product should feel operational and tool-like.
- Use **Korean-first copy** with short, explicit labels: `실시간 자막`, `회의 요약`, `전체 대화`, `저장하고 종료`, `요약하기`.
- Support transcript correction directly in the product. A user must be able to open the full transcript, edit it, and regenerate the summary from the edited version.
- Recent history should remain compact and secondary. The live recording workspace is always the primary focus.
