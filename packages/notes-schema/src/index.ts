...
export function getNotesPrompt(mode: SessionMode): string {
  ...
  const modeSpecific = {
    meeting: [
      ...
      "topicTimeline must be chronological by startMs and explain what topic was discussed, what was said, and what outcome or unresolved point remained in each topic block. It is source structure for a single user-facing 주제별 요약 section, not a separate report section.",
      ...
      "Use this reportMarkdown section order when content exists: 한 줄 결론, 보고용 요약, 회의 배경, 핵심 내용, 결론, 결정사항, 액션아이템, 미결사항, 리스크, 확인 필요 항목, 주제별 요약.",
      "Keep keywords as JSON metadata only; do not create a reportMarkdown section named 주요 키워드.",
      "Do not create separate reportMarkdown sections named 주제 흐름, 대화기록 미리보기, 원문 근거, or 근거.",
